...
import {
  getDatabase, ref, get, set, update, onValue, runTransaction, remove
...
async function attachRoom(code){
  ...
  unsubscribe=onValue(roomRef,async snap=>{
    const room=snap.val();
    if(!room){setStatus("الغرفة غير موجودة. تأكد من الكود أو أنشئ غرفة جديدة.");return}
    if(room.status==="ended"){setStatus("انتهت هذه الغرفة. أنشئ غرفة جديدة.");return}
    showPlayers(room);
    window.onlineNames={
...
window.joinOnlineRoom=async function(){
  try{
    ...
    const result=await runTransaction(r,room=>{
      if(!room)return;
      /* إذا كان هذا الجهاز دخل سابقًا، نسمح له بإعادة الاتصال. */
      if(room.guest && room.guest.uid!==uid)return;
      room.guest={uid,name:window.playerData.name||"اللاعب 2"};
      room.status="ready";
      return room;
    });
    if(!result.committed)throw new Error("الغرفة فيها لاعب ثانٍ بالفعل. أنشئ غرفة جديدة أو استخدم الجهاز الذي دخل أولًا.");
    showRoomCode(code); await attachRoom(code);
  }catch(e){setStatus("❌ "+e.message)}
};
...
window.leaveOnlineRoom=async function(){
  ...
  if(roomRef&&window.onlineSlot==="p1"){
    try{await update(roomRef,{status:"ended",endedAt:Date.now()})}catch{}
  }
